...
public class FragmentDefault extends FragmentBase {
    ...
    private DefaultMessagesReceiver defaultMessagesReceiver;
    ...
    private View.OnClickListener onFabButtonClicked = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            if (isDefaultMessenger()) {
                ...
                Toast.makeText(context, "Default messenger  is not installed.", Toast.LENGTH_SHORT).show();
            }
        }
    };

    public boolean isDefaultMessenger() {
        try {
            context.getPackageManager().getApplicationInfo(Telephony.Sms.getDefaultSmsPackage(context), 0)
            ;
            return true;
    ...
    private void iniRecyclerView() {
        recyclerView = view.findViewById(R.id.recycler_view_fr_default);
    ...
    private void getMessageInBackgroundTask() {
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                ...
                usersList.addAll(myDataBaseHelper.getALLUsers(TableName.TABLE_NAME_USER_DEFAULT));
                return null;
            }
    ...
    private void updateTableMessages(int position) {
        ...
        List<Messages> list = myDataBaseHelper.getSelectedMessages(TableName.TABLE_NAME_MESSAGES_DEFAULT, currentUser.getUserTitle());
        list.get(list.size() - 1).setReadStatus("Read");
        Messages messages = list.get(list.size() - 1);
        myDataBaseHelper.updateMessages(TableName.TABLE_NAME_MESSAGES_DEFAULT, messages);
    }
    ...
    private void registerReceiver() {
        defaultMessagesReceiver = new DefaultMessagesReceiver();
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(Constant.ACTION_INTENT_FILTER_DEFAULT_RECEIVER);
        getContext().registerReceiver(defaultMessagesReceiver, intentFilter);
    }

    public class DefaultMessagesReceiver extends BroadcastReceiver {
        @Override
